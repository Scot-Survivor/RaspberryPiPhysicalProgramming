...
#include "mfrc522.h"
#include "dump.h"
int MFRC522_Debug_DumpSector(uint8_t *CardID, uint8_t block_addr, uint8_t *SectorKeyA) {
	int ret, i;
	uint8_t buffer[1024] = "";

	printf(
            "Auth Block (0x%02X) with key 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X ...",
            block_addr, SectorKeyA[0], SectorKeyA[1], SectorKeyA[2], SectorKeyA[3],
            SectorKeyA[4]);
...
int MFRC522_Debug_CardDump(uint8_t *CardID, uint8_t *SectorKeyA) {
	int ret_int = 0;

	{
		int i;
		for (i = 0x0; i < 0x40; i += 4) {
			ret_int |= MFRC522_Debug_DumpSector(CardID, i, SectorKeyA);
		}
	}
	return ret_int;
}
